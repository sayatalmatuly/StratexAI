...
def should_force_web_search(message, conversation_history=None):
    m = (message or "").lower().strip()
    if not m:
        return False
    hard_phrases = [
        "посмотри в интернете", "найди в интернете", "поищи в интернете", "ищи в интернете", "узнай в интернете",
        "посмотри в сети", "найди в сети", "узнай в сети",
        "search the web", "look up on the internet", "find on the internet", "check online", "look it up online"
    ]
    if any(p in m for p in hard_phrases):
        return True
    internet_markers = ["интернет", "в интернете", "в сети", "web", "online", "internet"]
    local_context_markers = [
        "в этом", "этот", "эту", "это", "ниже", "выше", "в тексте", "в сообщении", "в чате",
        "в документе", "в файле", "в коде", "из файла", "из документа",
        "in this", "below", "above", "in the text", "in this chat", "in this message", "in this code", "in this file"
    ]
    discovery_verbs = ["найди", "узнай", "поищи", "ищи", "find", "look up", "search", "check"]
    freshness_markers = ["актуаль", "сейчас", "сегодня", "последн", "новости", "current", "latest", "today", "now", "up-to-date"]
    has_verb = any(v in m for v in discovery_verbs)
    if not has_verb:
        return False
    if any(marker in m for marker in local_context_markers):
        return False
    if any(marker in m for marker in internet_markers):
        return True
    if any(marker in m for marker in freshness_markers):
        return True
    if conversation_history:
        recent = " ".join((item.get("content", "") or "").lower() for item in conversation_history[-3:] if item.get("role") == "user")
        if any(marker in recent for marker in local_context_markers):
            return False
    return False
...
def chat():
    try:
        ...
        force_web_search = should_force_web_search(message, conversation_history)
        use_web = bool(data.get("use_web", False) or force_web_search)
        if force_web_search:
...
Use context from previous messages when relevant."""
            response = chat_with_ai(message, system_prompt, conversation_history[-10:])
